...
def plot_res(df, outpath, show=False):
    ...
    print("file was saved "+ outpath + '.png')
    if show:
        plt.show()
...
def run(samp_path,samp_order):
    ...
    df = pd.read_csv(samp_path,index_col=0, sep='\t')
    #rename Erythrocyte progenitors to Megakaryocytes
    df.rename(index={'Erythrocyte_progenitors':'Megakaryocytes'},inplace=True)
    ...
    # Plot pie charts
    outpath = samp_path.split('/')[-1]
    plot_res(df, outpath[:-len("_deconv_output.csv")])
